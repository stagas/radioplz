...
function nStore(filename, filterFn, isTemp) {
  ...
  function checkQueue() {
    ...
    while (i < length && size < 50000) {
      ...
      var meta = item.meta ? item.meta : {key: key};
      var line = item.line ? item.line : new Buffer(data + "\t" + JSON.stringify(meta) + "\n");
      var dataLength = (typeof item.length !== 'undefined') ? item.length : Buffer.byteLength(data);
